...
/**
 * A filter which use FFTs to simulate lens blur on an com.jhlabs.image.
 */
...
//                System.out.println("Tile: "+tileX+" "+tileY+" "+tileWidth+" "+tileHeight);
                ...
                // Clip the tile to the com.jhlabs.image bounds
                ...
                // Create a float array from the pixels. Any pixels off the edge of the source com.jhlabs.image get duplicated from the edge.
                i = 0;
                for ( int y = 0; y < h; y++ ) {
                    int imageY = y+tileY;
                    int j;
...
//							b = bloomThreshold + (b-bloomThreshold) * bloom;
                ...
                //FIXME-don't bother converting pixels off com.jhlabs.image edges
                for ( int y = 0; y < w; y++ ) {
                    int ym = y ^ row_flip;
                    int yi = ym*cols;
                    for ( int x = 0; x < w; x++ ) {
                ...
                // Clip to the output com.jhlabs.image
                tx = tileX+iradius;
                ty = tileY+iradius;
